...
const Header = (props) => {
  ...
  const handleClickOpen = async (e) => {
    ...
    try {
      ...
      console.log("Cannot connect to server. Start server: ", e.message);
    }
  };
  ...
  const linkMaker = (item) => {
    return item.toLowerCase() === "home"
      ? ""
      : item.toLowerCase().replaceAll(/\s/g, "");
  };
  ...
  const navItemList = (items) => {
    return (
      <List>
        {items.map((item) => (
          <ListItem>
            <ListItemText classes={{ root: classesItemText.root }}>
              <Link
                to={`/${linkMaker(item)}`}
                style={{
                  textDecoration: "none",
      ...
      <AppBar position="static" className={classes.toolbar.root} id="top">
        <Toolbar classes={toolbarStyles}>
          ...
          {data === undefined || !data.isAuthenticated ? (
            <Form open={loginOpen} onClose={handleClose} />
          ) : (
            <UserMenu
              anchorEl={anchorEl}
